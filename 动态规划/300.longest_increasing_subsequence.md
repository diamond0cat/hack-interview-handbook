...
class Solution {
    public int lengthOfLIS(int[] nums) {
        int len = nums.length;
        int[] memo = new int[len]; //initalize to special value:0，表示没算过，只要算过，就一定不是0
    ...
    private int longest(int[] arr, int index, int[] memo) {
        if (memo[index] != 0) return memo[index];
        int max =1;
        for (int i = 0; i < index; i++) {
            if (arr[i] <arr[index]) {
                max = Math.max(max, longest(arr, i, memo) + 1);
            }
        }
        memo[index] = max;
        return max; 
...
V`    public int lengthOfLIS(int[] nums) {
